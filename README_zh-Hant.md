...
### v1.9.0 - 備註
- **備註：** 直接在懸浮視窗為任何檔案加上個人備註 —— 備註會隨檔案一起儲存（擴充屬性），跨重啟保留，並在複製或移動檔案時一併帶走
- **自動調整高度：** 至少 3 行，並隨輸入內容增長；編輯時呈現聚焦的輸入框樣式
- **更穩定：** 輔助功能查詢移至主執行緒之外（Finder 反應慢時不再卡住畫面），並更精簡、安全地讀取大型檔案，以及多項修正
- **修正：** 聚焦 Liquid Glass 懸浮視窗時背後出現的黑色邊角
- 🙏 **非常感謝 [@zmlim](https://github.com/zmlim)** 設計並貢獻備註功能（[#14](../../pull/14)）—— 本次發行建立在他的成果之上
...
> **注意：** 不支援 macOS 26.4 Beta 1，請更新至 Beta 2 或更新版本。
...
> **已知問題（macOS 27 beta）：** 目前在 macOS 27 beta 上，設定介面會有顯示不正確的問題，但主要功能皆正常運作。
...
感謝所有貢獻者 —— 特別感謝 [@zmlim](https://github.com/zmlim) 設計並貢獻了本次 v1.9.0 的主打功能「備註」（[#14](../../pull/14)）。
