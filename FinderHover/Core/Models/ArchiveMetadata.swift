...
struct ArchiveMetadata {
    ...
    let uncompressedSize: Int64?  // Total uncompressed size
    ...
    var formattedCompressionRatio: String? { compressionRatio.map { String(format: "%.1f%%", $0) } }
...
struct DiskImageMetadata {
    ...
    let compressionRatio: Double? // Compression ratio (e.g., 2.5 → "2.5:1")
    ...
    var formattedCompressionRatio: String? { compressionRatio.map { String(format: "%.1f:1", $0) } }
